...
class Visualizer():
    def __init__(self, model, model_dir,
                 ...
                 display_loss_per_dim=False, # corresponds to ~2 for standard normal
                 upsample_factor=1):
        """
    ...
    def reconstruct_image(self,img,slid):
     
        post_mean, post_logvar = self.model.encoder(img[None, ...])
        samples = self.model.reparameterize(post_mean, post_logvar)
        for i in range(samples.size(0)):  # loop over mini-batch dimension
             t = samples[i]
                
        t[21] += float(slid[0][1].get())
        t[22] += float(slid[1][1].get())
        t[16] += float(slid[2][1].get())
        t[19] += float(slid[3][1].get())
        t[13] += float(slid[4][1].get())
        t[3] += float(slid[5][1].get())
        t[12] += float(slid[6][1].get())
        t[9] += float(slid[7][1].get())
        t = t.reshape(1,32)
        
        test= self.model.decoder( t).cpu()
        test = test.detach().numpy()
        test = test[0].reshape(64,64)
        test = Image.fromarray((test * 255).astype(np.uint8))
        test.save('C:/Users/Tasiko/Thesis/image generator/temp/temp_reconstruction/temp_1.png')
