...
def get_sctterplot_dataloader(batch_size, path_to_data):
    """CelebA dataloader with (64, 64) images."""
    scatterplot_data = ScatterplotDataset(path_to_data,
                                transform=transforms.ToTensor())
    scatterplot_loader = DataLoader(scatterplot_data, batch_size=batch_size,
                               shuffle=True)
    return scatterplot_loader
...
class ScatterplotDataset(Dataset):
    """Scatterplot dataset with 64 by 64 images."""
    def __init__(self, path_to_data, subsample=1, transform=None):
        """
        Parameters
        ----------
